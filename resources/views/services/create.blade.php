...
<section class="content-header">
    <h1>Add New Subscription
    <div class="pull-right">
        <?php
        $id = "";
        if(isset($_GET['id'])){
            $data = explode(",", $_GET['id']);
            if(isset($_GET['id']))   $id = "?id=".$data[0];
        }
          ?>
        <a class="btn btn-primary" href="{{ url('services'.$id) }}"> Back</a>
    </div></h1>
</section>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box" >
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" id="orderForm" method="post" action="{{ url('save_service') }}" autocomplete="on" enctype="multipart/form-data">
                    {{ csrf_field() }}
                  <div class="box-body">
                      ...
                      <div class="col-sm-10">
                          <select class="form-control select2" id="vertical_id" name="vertical_id" required >
                            ...
                            <option value="{{$vert->id}}" <?php if(isset($data[1])) { if($data[1] == $vert->id) echo "selected"; } ?>>{{$vert->vertical_name}}</option>
                            @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <select class="form-control select2" id="category_id" name="category_id" required >
                            ...
                            <option value="{{$cat->id}}" <?php if(isset($data[0])) { if($data[0] == $cat->id) echo "selected"; } ?>>{{$cat->category_name}}</option>
                            @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="form-group">
                      <label for="inputEmail3" class="col-sm-2 control-label">Subscription Name <span style="color:red"> * </span></label>

                      <div class="col-sm-10">
                          <input type="text" name="service_name" id="service_name" class="form-control" rows="3" placeholder="Subscription Name" required>
                      </div>
                    </div>
    ...
    var k = 1;
     $(".append").click( function(e) {
        ...
        $("#type_of_service").on("change",function(){
            var id = $(this).val();
            var v_id = $("#vertical_id").val();
            var c_id = $("#category_id").val();
            $.ajax({
                url: 'get_service_id/'+id+'/'+v_id+'/'+c_id,
                type: "GET",
                success: function (data) {
                    console.log(data);
                    if(data != ""){
                        alert(data);
                        $("#type_of_service").val("").attr("selected","selected");
